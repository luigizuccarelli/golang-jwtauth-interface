...
func jwtFields(jwtUser jwt.MapClaims, conn connectors.Clients) schema.UserPermissons {
	...
	if jwtUser["contactly_skip_role_check"] != nil {
		isSkipRoleCheck = jwtUser["contactly_skip_role_check"].(bool)
		conn.Trace("claims func, JWT contactly_skip_role_check claim: %t", isSkipRoleCheck)
	}
