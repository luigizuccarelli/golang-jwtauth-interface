...
import (
	...
	"lmzsoftware.com/lzuccarelli/common-auth-interface/pkg/schema"
)
...
func TestAllMiddleware(t *testing.T) {
	...
	t.Run("AuthHandler : should pass", func(t *testing.T) {
		...
		req, _ := http.NewRequest("GET", "/api/v1/verify", nil)
		con := connectors.NewTestConnectors(logger)
		req.Header.Set("X-Original-Uri", "/osmservice/api/v1/list/jobs")
		handler := http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
			AuthHandler(w, req, con)
		})
	...
	t.Run(testName, func(t *testing.T) {
		...
		resources := []string{"/osmservice/api/v1/jobs/1234",
			"/orderservice/api/v1/orders/customer/1234/owningorg/A1V21",
			...
			"/oneloginservice/api/v1/refresh/122629887",
		}
...
func createJWT(userPermissions schema.UserPermissons, isSkipRoleCheckNeeded bool) (jwt.MapClaims, *jwt.Token) {
	...
	token := jwt.NewWithClaims(jwt.SigningMethodHS256, jwt.MapClaims{
		...
		"contactly_skip_role_check": isSkipRoleCheckNeeded,
	})
